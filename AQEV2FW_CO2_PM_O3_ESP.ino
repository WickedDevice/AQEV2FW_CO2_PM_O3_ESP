...
#define EEPROM_O3_SENSITIVITY     (EEPROM_MQTT_PASSWORD - 4)      // float value, 4-bytes, the sensitivity from the sticker
...
#define EEPROM_CO_SENSITIVITY     (EEPROM_O3_CAL_OFFSET - 4)     // float value, 4-bytes, the sensitivity from the sticker
...
#define EEPROM_O3_BASELINE_VOLTAGE_TABLE (EEPROM_NTP_TZ_OFFSET_HRS - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
#define EEPROM_CO2_BASELINE_VOLTAGE_TABLE (EEPROM_O3_BASELINE_VOLTAGE_TABLE - (5*sizeof(baseline_voltage_t))) // array of (up to) five structures for baseline offset characterization over temperature
...
#define EEPROM_O3_ZERO_NEGATIVE_RESULTS (EEPROM_DISABLE_SOFTAP - 1)
#define EEPROM_CO_ZERO_NEGATIVE_RESULTS (EEPROM_O3_ZERO_NEGATIVE_RESULTS - 1)
...
#define EEPROM_BACKUP_CO2_SENSITIVITY    (EEPROM_BACKUP_O3_CAL_OFFSET + 4)
...
#define EEPROM_BACKUP_O3_SENSITIVITY     (EEPROM_BACKUP_MQTT_PASSWORD + 32)
...
#define BACKUP_STATUS_O3_CALIBRATION_BIT          (5)
...
void initializeHardware(void) {
    ...
    Serial.println(F(" |     O3 / CO2 / PM Sensor Suite     |"));
...
void restore(char * arg) {
    ...
    if (strncmp(arg, "defaults", 8) == 0) {
        ...
        eeprom_write_block("aqev2_co2_pm_o3_esp", (void *) EEPROM_UPDATE_FILENAME, 32);
    }
...
boolean publishHeartbeat() {
    ...
    snprintf(scratch, SCRATCH_BUFFER_SIZE-1,
             ...
             "\"converted-value\":%d,"
             "\"firmware-version\":\"%s\","
             "\"publishes\":[\"co2\",\"particulate\",\"o3\",\"temperature\",\"humidity\"%s],"
             "\"counter\":%lu"
